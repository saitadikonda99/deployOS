...
- **`internal/`** holds the implementation, split by concern
  ...
  `devices`, `auth`, `docker`, `scheduler`, `discovery`, `secrets`, ...).
  It is unimportable outside this module, which is exactly the point:
  these are DeployOS's own internals, not a public API.
...
[connection.md](./connection.md)) - the transport future features
(heartbeats, command delivery, metrics, log streaming) are expected to
...
and the persistent authenticated gRPC connection (see
[connection.md](./connection.md)) are implemented. Heartbeats, command
execution, Docker integration, deployments, HTTPS, secrets, databases,
