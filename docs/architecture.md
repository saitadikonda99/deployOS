...
- **`internal/`** holds the implementation, split by concern
  ...
  `commandbus`, `devices`, `auth`, `docker`, `scheduler`, `discovery`,
...
([connection.md](./connection.md)), and the Command Bus
([command-bus.md](./command-bus.md), with `PING`/`GET_VERSION`/`GET_INFO`
as its only commands so far) are implemented. Heartbeats, Docker
integration, deployments, HTTPS, secrets, databases, monitoring, backups,
