...
`internal/applications` (see [application-engine.md](./application-engine.md))
sits a layer above `internal/containers`, not beside it: an
`Application` is desired state (what should run), a
`containers.Container` is observed state (what a Runtime Engine
provider reports actually running). `internal/applications` depends on
nothing container-specific - its `Engine` interface is where a future
implementation will bridge the two, the same one-directional
